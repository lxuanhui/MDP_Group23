...
#host = "192.168.23.24" #algo brandon laptop
host = "192.168.23.35"
port = 1234
	...
	if (result == b'#'):
		...
		for data in datas: # Loop through all the commands
			...
			if data["reached"] == 0: #Havent reach obstacle, send stm commands
				...
				check_next_list = []
				while (x != b'next'):
				#while (b'next' not in check_next_list):
					x = serSTM.readline()
					#check_next_list.append(x)
					resend += 1
					if (resend == 30):
				...
				#check_next_list.clear()
				x = b''
			else:
				dummyValue = data["obstacle"] * 10
				serBluetooth.write(str.encode(r'{{"target":[{0},{1}]}}'.format(data["obstacle"],dummyValue)))
			...
			#		serBluetooth.write(str.encode(r'{{"target":[{0},{1}]}}'.format(data["o"],rec_image)))##SEND TO BT
