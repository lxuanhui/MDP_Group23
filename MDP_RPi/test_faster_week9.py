...
#		serSTM.write(b'w100)')
		...
		while (count != 3):
			...
			while (x != b'next'): #Wait for STM to get the first image
				...
				#	break  
				if ((countResend == 2 and first == 1) or reCheck == 1): #Resent STM twice for first movement
					#Take image, for faster processing
...
#                      			 capture = 1 # capture photo     
					...
					while rec_image not in ["99", "39", "38"]:	
						with open('imagerec_result.txt') as s:
							rec_image = s.read()
					if (rec_image != "39" or rec_image != "38"):
						reCheck = 1 # Do reCheck take image.
					else:
						reCheck = 0
						firstImage = rec_image
